...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            # --- SAVE FINAL SCAN IMAGE ---
            if data['type'] == 'scan_result_image':
                try:
                    img_b64 = data['image'].split(',')[1]
                    img_bytes = base64.b64decode(img_b64)
                    os.makedirs("data", exist_ok=True)
                    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                    save_path = os.path.join("data", f"scan_{timestamp}.png")
                    with open(save_path, "wb") as f:
                        f.write(img_bytes)
                    print(f"[✓] Scan image saved: {save_path}")
                except Exception as e:
                    print(f"[!] Failed to save scan image: {e}")
                continue
            ...
            if data['type'] == 'frame' and scan_start_time is not None:
                ...
                if not face_found:
                    ...
                    if M["m00"] != 0:
                        ...
                        if prev_face_center is None:
                            ...
                            if np.sqrt((cx - prev_face_center[0])**2 + (cy - prev_face_center[1])**2) > 80.0:
                                motion_detected = True
                ...
                if elapsed < 5.0:
                    ...
                    if results and results.get('hr'):
                        ...
                        # --- ONLY SEND GRAPH ON CONVERGED ---
                        if phase == "CONVERGED" and results.get('spectrum'):
                            response['spectrum'] = results['spectrum']
